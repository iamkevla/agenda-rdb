 ...
 *  NF 20/04/2015
 */
Agenda.prototype.dbInit = function(db, table, cb) {
    this._table = r.table(table || 'agendaJobs');
    var self = this;

    r.dbCreate(db).run(function(err) {
        r.db(db).tableCreate(table).run(function(err) {
            r.db(db).table(table).indexCreate('findAndLockNextJobIndex1', [
                r.row('name'),
                r.row('priority'),
                r.row('lockedAt'),
                r.row('nextRunAt'),
...
Agenda.prototype.saveJob = function(job, cb) {
    ...
    if (id) {
        ...
        if (uniqueOpts && uniqueOpts.insertOnly) {
            ...
            self._table.filter(query).run().then(function(results) {
                if (results.length) {
                    self._table.filter(query).update(props, {
                        returnChanges: true
                    }).run(processDbResult);
                } else {
                    self._table.insert(props, {
...
Agenda.prototype._unlockJobs = function(done) {
    ...
    if (jobIds.length) {
        ...
        this._table
            .filter(function(row) {
                return r.expr(jobIds).contains(row('id'));
            })
            .update({
                lockedAt: null
            }, {
