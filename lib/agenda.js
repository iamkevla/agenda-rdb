...
Agenda.prototype.rethink = function (rdb, table, cb) {
    r = this._rdb = rdb;
    this.db_init(table, cb); // NF 20/04/2015
    return this;
};
 ...
 *  NF 20/04/2015
 */
Agenda.prototype.db_init = function (table, cb) {
    this._table = this._rdb.table(table || 'agendaJobs');
    var self = this;
    this._table.indexCreate("findAndLockNextJobIndex1", [
        this._rdb.row("name"),
        this._rdb.row("priority"),
        this._rdb.row("lockedAt"),
        this._rdb.row("nextRunAt"),
        this._rdb.row("disabled")]).run(function (err, result) {
...
Agenda.prototype.saveJob = function (job, cb) {
    ...
    if (id) {
        this._table.filter({
            id: id
        }).replace(props, {returnChanges: true}).run(processDbResult);
        ...
        // Try an upsert.
        this._table.insert(props, {conflict: 'replace', returnChanges: true}).run(processDbResult);
    } else if (unique) {

        var query = job.attrs.unique;
        query.name = props.name;
        if( uniqueOpts && uniqueOpts.insertOnly ) {
            this._table.insert(props, { returnChanges: true}).run(processDbResult);
        } else {
            self._table.filter(query).coerceTo('array')
                .do(function(matches) {
                    return r.branch(
                        matches.eq(null),
                        self._table.insert(props, {returnChanges: true}),
                        self._table.filter(query).replace(props, {returnChanges: true})
                    );
                })
                .run(processDbResult);
        ...
        this._table.insert(props, {returnChanges: true}).run(processDbResult); // NF updated 22/04/2015
    }
...
Agenda.prototype._findAndLockNextJob = function (jobName, definition, cb) {
    ...
    if (this._rdb.getPoolMaster().getLength() === 0) {
        cb(new Error('No RethinkDB Connection'));
    } else {

         this._table.filter(function(job){
            return job('name').eq(jobName)
                .and(job('disabled').ne(true))
                .and(job('nextRunAt').le(Date(this._nextScanAt)))
                .and(job('lockedAt').eq(null).or(job('lockedAt').le(lockDeadline)));
            })
            .update({
                lockedAt: this._rdb.now()
            }, {returnChanges: true}).run(function (error, result) {
                var jobs;

                if (!error && result.changes && result.changes[0].new_value) {
                    jobs = createJob(self, result.changes[0].new_value);
                }
                cb(error, jobs);
            });
    }
...
Agenda.prototype._unlockJobs = function (done) {
    ...
    this._table.getAll(jobIds, {index: 'id'})
        .update({
            lockedAt: null
        })
        .run(done); 
...
function processJobs(extraJob) {
    ...
    if (!extraJob) {
        ...
        self._table.filter({
            ...
            lockedAt: this._rdb.now()
        },{returnChanges: true}).run(function (err, resp) {
        ...
        self._findAndLockNextJob(name, definitions[name], function (err, jobs) {
            ...
            if (jobs) {
                ...
                for (var job in jobs) {
                    ...
                    for (var queued_job in jobQueue) {
