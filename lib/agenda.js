...
var Agenda = module.exports = function (config, cb) {
    ...
    if (config.rethinkdb) {
        ...
        this.database(config.db.config, config.db.config.table, config.db.options, cb);
    }
};
 ...
 */
Agenda.prototype.database = function (config, table, options, cb) {
...
Agenda.prototype.saveJob = function (job, cb) {
    ...
    if (id) {
        this._table.filter({
            id: id
        }).replace(props).run(processDbResult);
    ...
    if (this._mdb.s.topology.connections().length === 0) {
        ...
        this._table.filter(function(job){
            return job('name').eq(jobName)
                ...
                .and(job('nextRunAt').lte(this._nextSsanAt))
                and(job('lockedAt').eq(null).or(job('lockedAt').lte(lockDeadline)));
        }).update({
            lockedAt: r.now()
        }).run(function (error, result) {
            var jobs;
            if (!error && result.changes[0].new_value) {
...
function processJobs(extraJob) {
    ...
    if (!extraJob) {
        ...
        self._table.filter({
            ...
            if (resp.changes[0].new_val) { // NF 20/04/2015
