...
Agenda.prototype.saveJob = function(job, cb) {
    ...
    if (id) {
        ...
        // Try an upsert.
        self._table.filter({name: props.name, type: 'single'}).run().then(function(results) {
            if (results.length) {
                self._table.filter({name: props.name, type: 'single'}).replace(props, {
                    returnChanges: true
                }).run(processDbResult);
            } else {
                self._table.insert(props, {
                    returnChanges: true
                }).run(processDbResult);
            }
        });
        ...
        if (uniqueOpts && uniqueOpts.insertOnly) {
            ...
            self._table.filter(query).run().then(function(results) {
                if (results.length) {
                    self._table.filter(query).replace(props, {
                        returnChanges: true
                    }).run(processDbResult);
                } else {
                    self._table.insert(props, {
