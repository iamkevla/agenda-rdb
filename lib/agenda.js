...
var Agenda = module.exports = function (config, cb) {
    ...
    if (config.rethinkdb) {
        this.rethink(config.rethink, config.db ? config.db.table : undefined, cb);
    } else if (config.db) {
        this.database(config.db.url, config.db.table, config.db.options, cb);
    }
};
...
Agenda.prototype.saveJob = function (job, cb) {
    ...
    if (id) {
        ...
        this._collection.findAndModify(query, {}, update, {
            upsert: true,
            new: true
        }, processDbResult);
    } else {
